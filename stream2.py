...
class MaxListener(tweepy.StreamListener):
    ...
    def process_data(self, raw_data):
        ...
        #csvWriter.writerow([decoded_data["id"],decoded_data["text"].encode('utf-8')])
...
class Ranking():
    ...
    flagged_words = ["antifa", "bluelivesmatter", "blue lives matter", "#bluelivesmatter", "nigger","trump", "nigga", "niggers", "niggas"]
        ...
        for xyz in tweets:
            ...
            for word in tweet_content:
                ...
                if word.lower() in self.police_markers:
                    score += 1
                if word.lower() in self.medical_and_supplies_markers:
                    score += 1
    ...
    def retweet(self):
        ...
        Retweets the best ranked tweets 
        """
        self.check()
        self.ranked_tweets.sort(key=lambda x: x[1], reverse=True)
        for i in range(5): #how much it tweets in interval
            try:
...
if __name__=="__main__":
    ...
    #stream.start(["#blmprotests, BLM protest, #newyorkprotest, #neworleansprotest, #chicagoprotests"])
    stream.start(["#neworleansprotest, #chicagoprotests, #newyorkprotest"])
    trial = Ranking()
    while True:
